...
const Principal = () => {
    ...
    const handleNoteClick = (index) => {
        setSelectedNote(localStorageData[index]);
    };
    ...
    return (
        <>
            <article className="flex flex-col items-center justify-center py-4 gap-10">
                    ...
                    {localStorageData.length > 0 ? (
                        localStorageData.slice(0).reverse().map((task, index) => (
                            ...
                            <div key={index} onClick={() => handleNoteClick(index)}>
                             <Link to={{ pathname: "/Detalles", state: { selectedNote: task } }} className="flex flex-row bg-gray-200 py-4 h-8/12 rounded-lg items-center justify-between px-6">
                                    ...
                                    <button onClick={() => handleNoteClick(index)}>Edit</button>
                                </div>
                             ) : null
                        ))
                    ) : (
                ...
                <div className="flex flex-col gap-4">
                    {/* Itera sobre las notas y muestra cada una */}
